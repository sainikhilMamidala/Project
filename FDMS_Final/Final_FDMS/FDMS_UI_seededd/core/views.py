...
def edit_donation(request, pk):
    ...
    if request.method == "POST":
        form = DonationForm(request.POST, instance=donation, user=request.user)
        if form.is_valid():
            edited = form.save(commit=False)

            # If they edited the initial quantity, adjust available qty only if no claims exist
            if 'initial_quantity' in form.changed_data:
                if donation.claims.exists():
                    messages.error(request, "You cannot change the initial quantity after claims exist.")
                    return redirect('edit_donation', pk=donation.pk)
                else:
                    edited.available_quantity = edited.initial_quantity
        ...
        form = DonationForm(instance=donation, user=request.user)
